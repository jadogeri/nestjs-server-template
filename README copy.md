...
Presentation Layer: Controllers & DTOs (Request handling & Validation).
Security Layer: Guards & Strategies (Authentication & RBAC).
Application Layer: Services (Business logic orchestration).
Infrastructure Layer: External Integrations (Mailers, SMS, S3).
Persistence Layer: Repositories & Entities (Database interactions).
